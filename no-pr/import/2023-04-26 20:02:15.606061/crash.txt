Traceback (most recent call last):
  ...
  File "/Python-3.11.1/Lib/asyncio/base_events.py", line 653, in run_until_complete
    return future.result()
           ^^^^^^^^^^^^^^^
  File "/app/iambic/config/dynamic_config.py", line 461, in load_config
    return await process_config(
           ^^^^^^^^^^^^^^^^^^^^^
  File "/app/iambic/config/dynamic_config.py", line 499, in process_config
    await config.configure_plugins()
  File "/app/iambic/config/dynamic_config.py", line 347, in configure_plugins
    await plugin.async_load_callable(self.get_config_plugin(plugin))
  File "/app/iambic/plugins/v0_1_0/aws/handlers.py", line 78, in load
    orgs_accounts = await asyncio.gather(
                    ^^^^^^^^^^^^^^^^^^^^^
  File "/app/iambic/plugins/v0_1_0/aws/models.py", line 649, in get_accounts
    session = await self.get_boto3_session()
              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/iambic/plugins/v0_1_0/aws/models.py", line 208, in get_boto3_session
    boto3_session = await create_assume_role_session(
                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/iambic/plugins/v0_1_0/aws/utils.py", line 270, in create_assume_role_session
    role = await aio_wrapper(sts.assume_role, **role_params)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/app/iambic/core/utils.py", line 170, in aio_wrapper
